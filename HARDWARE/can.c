...
void CAN1_RX0_IRQHandler(void)
{
	u32 id;
	u8 ide,rtr,len;   
	float temp1,temp2,temp3,step1=0.01,step2=0.005;
	static float lasttemp1,lasttemp2;
	...
	if(id==0x00000001)
	{
		...
		//计算三轴目标速度，单位：m/s
		temp1=((float)((short)((temp_rxbuf[0])+(temp_rxbuf[1]*0x100)))-2048)/1000*0.5; //(-1600~1600)/1000*0.5
		temp2=((float)((short)((temp_rxbuf[2])+(temp_rxbuf[3]*0x100)))-2048)/1000*0.5;
		temp3=0;

		if(fabs(temp1)<0.005) temp1=0;
		if(fabs(temp2)<0.005) temp2=0;
		if(temp1!=0 || temp2!=0) Control_mode=0x03;
		//else if(USART_GetITStatus(UART5, USART_IT_RXNE) == RESET && temp1==0 && temp2==0) Control_mode=0x01;
		
		
		if(Control_mode==0x03 || Control_mode == 0x04)
		{
			if(temp1>0)
			{
				temp1=temp1*1.25*Speed_Limit;
				temp3=1;
			}
			else if(temp1<0)
			{
			...
			if(sqrt(temp1*temp1 + temp2*temp2) > Speed_Limit)
			{
				cur_V_Speed=temp3 * sqrt(Speed_Limit*Speed_Limit-temp2*temp2);
				cur_W_Speed=temp2;
				Joy_v_Speed=cur_V_Speed;
				Joy_w_Speed=cur_W_Speed;
			}
			else
			{
				cur_V_Speed=temp1;
		...
		//lasttemp1=cur_V_Speed;
		//lasttemp2=cur_W_Speed;
	}
}
#endif
